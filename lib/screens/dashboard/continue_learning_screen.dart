...
class _ContinueLearningScreenState extends State<ContinueLearningScreen> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      if (mounted) context.read<ContinueLearningProvider>().load();
    });
  }
  ...
  Future<void> _edit([ContinueLearningModel? item]) async {
    ...
    final progress = TextEditingController(
      text: (((item?.progress ?? 0) * 100).round()).toString(),
    );
    ...
    // Important: save only AFTER the dialog has closed.
    // Firestore's snapshot listener can notify the Provider immediately after
    // a write. Keeping the dialog open during that notification can trigger
    // Flutter's '_dependents.isEmpty' lifecycle assertion.
    ContinueLearningModel? result;

    try {
      result = await showDialog<ContinueLearningModel>(
        context: context,
        builder: (dialogContext) => StatefulBuilder(
          builder: (context, setDialogState) => AlertDialog(
            title: Text(
              item == null ? 'Add Continue Learning' : 'Edit Continue Learning',
            ),
            content: SizedBox(
              width: 520,
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(
                          value: 'none',
                          child: Text('No navigation'),
                        ),
                      ],
                      onChanged: (v) => setDialogState(
                        () => targetType = v ?? 'mockTest',
                      ),
                    ),
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  final p = ((double.tryParse(progress.text) ?? 0)
                              .clamp(0, 100) /
                          100)
                      .toDouble();
                  ...
                  final model = ContinueLearningModel(
                    ...
                    progress: p,
                    buttonText: button.text.trim().isEmpty
                        ? 'Continue'
                        : button.text.trim(),
      ...
      if (result != null && mounted) {
        await context.read<ContinueLearningProvider>().save(result);
      }
    } finally {
      for (final c in [
        title,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: p.loading
          ...
          : p.items.isEmpty
              ? const Center(
                  child: Text('No Continue Learning items. Add one from +'),
                )
              : ListView.builder(
                    ...
                    return Card(
                      child: ListTile(
                        leading: x.thumbnailUrl.isNotEmpty
                            ...
                            : const Icon(Icons.menu_book),
                        title: Text(
                          x.title.isEmpty ? '(Untitled)' : x.title,
                        ),
                        subtitle: Text(
                          '${x.subject}${x.chapter.isEmpty ? '' : ' • ${x.chapter}'} • $pct%',
                        ),
                        trailing: Wrap(
                          spacing: 0,
                          children: [

...
class _ContinueLearningScreenState extends State<ContinueLearningScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('$uploaded questions uploaded successfully.')),
        );
      }
    } catch (e) {
  ...
  Future<void> _deleteSet(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('The set will be removed. Questions inside its subcollection should also be removed before permanent cleanup.'),
        actions: [
  ...
  Widget _field(TextEditingController controller, String label, {String? hint, int maxLines = 1, TextInputType? keyboard}) => Padding(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Continue Learning')),
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('New Learning Set'),
      ),
      body: _busy
          ...
          : StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (docs.isEmpty) {
                  return const Center(child: Text('No learning sets. Tap + New Learning Set.'));
                }
                ...
                return ListView(
                  ...
                  children: [
                    ...
                    const Text('Create a set and upload 100–150 questions from Excel. Students will resume from their saved question.'),
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: Wrap(spacing: 4, children: [
          Switch(value: active, onChanged: (v) => _db.collection('continue_learning').doc(doc.id).update({'isActive': v})),
          IconButton(tooltip: 'Upload Excel', icon: const Icon(Icons.upload_file), onPressed: _busy ? null : () => _uploadExcel(doc.id)),
          IconButton(tooltip: 'Delete', icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteSet(doc.id)),
        ]),
      ),
    );
  }

...
class _ContinueLearningScreenState extends State<ContinueLearningScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$uploaded questions uploaded successfully.'),
  ...
  Future<void> _deleteSet(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'The set will be removed. Questions in its subcollection should also be cleaned up.',
        ),
        actions: [
          TextButton(
    ...
    if (ok == true) {
      await _db.collection('continue_learning').doc(id).delete();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Continue Learning Management'),
      ),
      ...
      body: _busy
          ...
          : StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                docs.sort(
                  (a, b) =>
                      ((a.data()['sortOrder'] as num?) ?? 0).compareTo(
                    (b.data()['sortOrder'] as num?) ?? 0,
                  ),
                );

                if (docs.isEmpty) {
                  return const Center(
                    child: Text('No learning sets. Tap + Add Learning Set.'),
                  );
                }
                ...
                return ListView(
                  ...
                  children: [
                    ...
                    const Text(
                      'Create a set and upload 100–150 questions from Excel. You can upload or replace questions any time using the Upload Excel button.',
                    ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: ListTile(
          ...
          subtitle: Text(
            '$count Questions${d['description'] == null || '${d['description']}'.isEmpty ? '' : '  •  ${d['description']}'}',
          ),
          trailing: Wrap(
            spacing: 4,
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: _busy ? null : () => _showEditSet(doc),
              ),
              FilledButton.icon(
              ...
              IconButton(
                ...
                onPressed: _busy ? null : () => _deleteSet(doc.id),
              ),
            ],
          ),
        ),

...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Container(
        decoration: const BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xffF8FAFC), Color(0xffEEF2FF), Color(0xffF0FDFA)])),
        child: LayoutBuilder(
          builder: (context, constraints) {
            final crossAxisCount = constraints.maxWidth >= 1100 ? 3 : constraints.maxWidth >= 650 ? 2 : 1;
            return GridView.builder(
              padding: const EdgeInsets.all(24),
              gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: crossAxisCount, crossAxisSpacing: 22, mainAxisSpacing: 22, childAspectRatio: 1.55),
              itemCount: cards.length,
              itemBuilder: (context, index) => DashboardCard(data: cards[index]),
            );
          },
        ),
      ),
...
class DashboardCardData {
  ...
  const DashboardCardData({required this.title, required this.count, required this.icon, required this.colors, required this.onTap});
}
      ...
      child: InkWell(
        ...
        child: Ink(
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(24),
            boxShadow: [BoxShadow(color: data.colors.first.withOpacity(.25), blurRadius: 18, offset: const Offset(0, 9))],
          ),
          child: Stack(
            children: [
              Positioned(right: -25, top: -25, child: Container(width: 120, height: 120, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(.12)))),
              Positioned(right: 20, bottom: 18, child: Icon(data.icon, size: 72, color: Colors.white.withOpacity(.18))),
              Padding(
                padding: const EdgeInsets.all(24),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                  Container(width: 58, height: 58, decoration: BoxDecoration(color: Colors.white.withOpacity(.20), borderRadius: BorderRadius.circular(17), border: Border.all(color: Colors.white.withOpacity(.28))), child: Icon(data.icon, color: Colors.white, size: 31)),
                  const SizedBox(height: 14),
                  Text(data.count, style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.w800)),
                  const SizedBox(height: 2),
                  Text(data.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 6),
                  Row(children: [Text('Open', style: TextStyle(color: Colors.white.withOpacity(.88), fontSize: 12, fontWeight: FontWeight.w600)), const SizedBox(width: 5), const Icon(Icons.arrow_forward_rounded, color: Colors.white, size: 16)]),
                ]),
              ),
            ],
          ),

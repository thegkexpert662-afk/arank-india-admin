    ...
    try {
      ...
      if (admin.exists) {
        ...
        if (data['isActive'] == false) {
          await _auth.signOut();
          return 'Your Admin account has been disabled by Master Admin.';
        }
        final appId = (data['instituteId'] ?? data['appId'] ?? '').toString();
        if (appId.isNotEmpty) {
          final app = await _db.collection('apps').doc(appId).get();
          if (app.exists && app.data()?['isActive'] == false) {
            await _auth.signOut();
            return 'Your Institute is currently disabled by Master Admin.';
          }
        }
      }
      return null;
